...
var Alignment = require('./character/alignment');
var Class = require('./character/class');
var Race = require('./character/race');
var Background = require('./background/background');
var Family = require('./background/family');
var Birth = require('./background/birth');
var Life = require('./background/life');
var Item = require('./item/item');

function sources(character) {
    if (character.sources) {
        character.sources = character.sources.split(',').map(function (s) {
            return s.trim();
        });
    } else {
...
function race(character) {
    if (character.race) {
        character.race = Race.byName(character.race);
    }
    if (!character.race) {
        character.race = Race.random(character.sources);
    }

    if (character.race.subraces.length > 0) {
        if (character.subrace) {
            character.subrace = character.race.subraces.filter(function (sr) {
                return sr.name === character.subrace;
            })[0];
        }
        if (!character.subrace) {
            character.subrace = Race.randomSubrace(character.race);
        }
    }
...
function alignment(character) {
    if (!character.alignment) {
        character.alignment = Alignment.random();
    }
    if (character.alignment) {
        character.alignment = Alignment.byAbbreviation(character.alignment);
    }
}
...
function background(character) {
    if (character.background) {
        character.background = Background.byName(character.background);
    }
    if (!character.background) {
        character.background = Background.random(character.sources);
    }
...
function adventuringClass(character) {
    if (character.class) {
        character.class = Class.byName(character.class);
    }
    if (!character.class) {
        character.class = Class.random(character.sources);
    }
...
function siblings(character) {
    character.siblings = [];
    for (var n = 0; n < Family.siblings(character.race); n++) {
...
function events(character) {
    character.events = [];
    var eventRolls = [];
    for (var n = 0; n < Life.eventCount(character.age); n++) {
...
module.exports = function () {
    ...
    var character = Object.assign({}, config);
